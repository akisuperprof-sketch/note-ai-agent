...
【執筆ルール（重要）】
1. **信頼感のある文体**: 「〜だなぁ」「〜かも」といった砕けすぎた表現や、「魔法」「激変」といった過度な煽りは避け、丁寧で誠実な「ですます調」で記述してください。読者に寄り添いつつも、プロとしての知見を感じさせる落ち着いたトーンを目指します。
2. **段落構成の徹底（最重要）**: 1文ごとの改行や、スカスカした行開けは**禁止**です。意味のまとまりごとに3〜5行程度の段落（パラグラフ）を作ってください。段落間には空行を入れますが、段落内では改行せず、文章を繋げてください。決して「詩」のような形式にしないでください。
3. **導入の構成**: 読者の課題への共感から入りつつ、本記事で得られる専門的な知見やメリットを明確に提示してください。
4. **共感と人間味**: AIっぽさを排除し、「私の経験では...」「正直に言うと...」といった主観的なエピソードや、筆者の体温を感じる表現を適度に織り交ぜてください。
